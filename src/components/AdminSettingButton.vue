...
<template>
    <div>
        <router-link v-if="!props.disabled" :to="{ name: props.link }"
            :class="[current ? 'bg-accent-500 hover:bg-accent-400' : 'bg-gray-100 dark:bg-gray-500 hover:bg-accent-400 hover:dark:bg-accent-600', 'flex items-left justify-between w-full p-2 rounded-lg mt-4 pl-6']">
